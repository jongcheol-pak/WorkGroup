...
- **앱 아이콘 추출**: Win32 앱은 셸 썸네일로, Store/UWP 패키지 앱은 셸이 렌더한 아이콘(`shell:AppsFolder\{AUMID}` + `IShellItemImageFactory`)을 추출한다 — 시작 메뉴와 동일해 앱별 로고 여백 편차 없이 균일. 추출 실패 시 단색/기본 아이콘으로 폴백.
