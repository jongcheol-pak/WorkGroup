...
- **그룹 구성**: "그룹 추가/수정" 다이얼로그에서 상단 [아이콘][이름(15자)], "앱 추가"로 설치 앱을 골라 선택 목록에 담고(항목별 삭제) 그룹 생성/편집/삭제. 그룹 아이콘은 클릭 → **사용자 이미지**(파일 선택) 또는 **리소스 아이콘**(번들 이미지 그리드) 선택. 앱이 없거나 이름이 중복이면 추가가 차단됨. **수정 모드**에서는 이름이 읽기전용으로 표시되고 클릭하면 입력창으로 전환되며, 이름을 바꾸면 "작업 표시줄에 핀한 기존 항목을 제거하고 다시 핀해야 한다"는 안내가 입력 영역 아래에 표시된다(핀 표시 이름은 Windows 제약상 자동 변경 불가).
