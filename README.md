...
- **앱 아이콘 추출**: Win32 앱은 셸 썸네일로, Store/UWP 패키지 앱은 셸 공식 로고(`AppListEntry.DisplayInfo.GetLogo`)를 AUMID로 직접 추출한다(`package.Logo` 경로가 없는 앱도 아이콘 표시). 추출 실패 시 단색/기본 아이콘으로 폴백.
