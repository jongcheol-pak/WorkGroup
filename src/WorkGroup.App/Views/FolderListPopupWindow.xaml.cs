...
public sealed partial class FolderListPopupWindow : Window
{
    ...
    private void ShowChildPopup(Button button, string path)
    {
        ...
        // 자식이 닫히면 추적만 해제(교체로 늦게 온 Closed는 무시). 닫기 판정은 1차 포커스가 담당.
        child.Closed += (_, _) =>
        {
    ...
    {
        // 1차만 포커스를 보유한다. 다른 앱/작업표시줄/바탕화면 클릭 → Deactivated → 전체 체인 닫기.
        // 자식 팝업은 Show(false)라 포커스를 뺏지 않으므로, 자식 표시/호버 중에는 1차가 활성 상태를 유지한다.
