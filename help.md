...
- **순서 변경** — 각 항목 맨 왼쪽의 **핸들(≡)**을 잡고 위아래로 끌면 목록 순서가 바뀌고 즉시 저장됩니다. (카드 본체를 끄는 것은 작업 표시줄 핀이므로 순서 변경은 핸들에서 시작해야 합니다. **검색 중에는 핸들이 숨겨집니다** — 검색어를 지운 뒤 정렬하세요.)

<img src="images/work-groups.png" alt="작업 그룹 화면 — 그룹 목록과 검색" width="640" />

#### 그룹 추가 / 수정 다이얼로그
- 상단에 **[그룹 아이콘] [이름(최대 20자)]**이 있습니다.
...
- **앱 순서 변경** — 선택 목록의 항목을 끌어 순서를 바꿀 수 있고, 그 순서대로 핀 팝업에 표시됩니다.
...
- **폴더 검색** — 등록된 폴더를 검색합니다.
- **순서 변경** — 각 항목 맨 왼쪽의 **핸들(≡)**을 잡고 끌면 목록 순서가 바뀌며, 트레이 좌클릭 팝업에도 같은 순서로 표시됩니다. (**검색 중에는 핸들이 숨겨집니다.**)
